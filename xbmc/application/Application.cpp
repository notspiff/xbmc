    ...
    m_WaitingExternalCalls(0)
{
  ...
  const auto appPlayerControl = std::make_shared<CApplicationPlayerControl>(m_bStop,
                                                                            *this);
  m_components.RegisterComponent(appPlayerControl);
  ...
  m_components.RegisterComponent(appPowerHandling);
  const auto appSkinHandling = std::make_shared<CApplicationSkinHandling>(this,
                                                                          this,
                                                                          m_bInitializing);
...
CApplication::~CApplication(void)
{
  ...
  m_components.DeregisterComponent(typeid(CApplicationPlayerControl));
  ...
  while (!m_portEvents.empty())
  {
    ...
    switch(newEvent.type)
    {
      ...
      case XBMC_SETFOCUS:
        ...
        m_components.GetComponent<CApplicationPowerHandling>()->ResetScreenSaver();
        m_components.GetComponent<CApplicationPowerHandling>()->WakeUpScreenSaverAndDPMS();
...
bool CApplication::CreateGUI()
{
  ...
  m_components.GetComponent<CApplicationPowerHandling>()->SetRenderGUI(true);
...
bool CApplication::Initialize()
{
  ...
  if (CServiceBroker::GetGUI()->GetWindowManager().Initialized())
  {
    ...
    m_components.GetComponent<CApplicationSkinHandling>()->m_confirmSkinChange = false;
    ...
    CServiceBroker::GetRenderSystem()->ShowSplash("");
    m_components.GetComponent<CApplicationSkinHandling>()->m_confirmSkinChange = true;
    ...
    if (!m_components.GetComponent<CApplicationSkinHandling>()->LoadSkin(skinId))
    {
      ...
      if (!m_components.GetComponent<CApplicationSkinHandling>()->LoadSkin(defaultSkin))
      {
  ...
  m_components.GetComponent<CApplicationPowerHandling>()->CheckOSScreenSaverInhibitionSetting();
...
void CApplication::Render()
{
  ...
  // render gui layer
  if (m_components.GetComponent<CApplicationPowerHandling>()->GetRenderGUI() && !m_skipGuiRender)
  {
...
bool CApplication::OnAction(const CAction &action)
{
  ...
  { // Switch to fullscreen mode if we can
    ...
    if (gui)
    {
      ...
      {
        m_components.GetComponent<CApplicationPowerHandling>()->m_navigationTimer.StartZero();
        return true;
      }
    }
  }
  ...
  const auto appPlayer = m_components.GetComponent<CApplicationPlayer>();
  const auto appPlayerControl = m_components.GetComponent<CApplicationPlayerControl>();
  ...
  if( !m_pInertialScrollingHandler->CheckForInertialScrolling(&action) )
  {
    ...
    {
      m_components.GetComponent<CApplicationPowerHandling>()->ResetNavigationTimer();
      return true;
    }
  }
  ...
  if (action.GetID() == ACTION_SWITCH_PLAYER)
  {
    ...
    if (appPlayer->IsPlaying())
    {
      ...
      if (!player.empty())
      {
        ...
        appPlayerControl->PlayFile(item, player, true);
      }
    }
    else
    {
      ...
      if (!player.empty())
      {
        appPlayerControl->PlayFile(CFileItem(), player, false);
      }
    }
  }
...
void CApplication::OnApplicationMessage(ThreadMessage* pMsg)
{
  ...
  case TMSG_SHUTDOWN:
    m_components.GetComponent<CApplicationPowerHandling>()->HandleShutdownMessage();
    break;
  ...
  case TMSG_INHIBITIDLESHUTDOWN:
    m_components.GetComponent<CApplicationPowerHandling>()->InhibitIdleShutdown(pMsg->param1 != 0);
    break;

  case TMSG_INHIBITSCREENSAVER:
    m_components.GetComponent<CApplicationPowerHandling>()->InhibitScreenSaver(pMsg->param1 != 0);
    break;

  case TMSG_ACTIVATESCREENSAVER:
    m_components.GetComponent<CApplicationPowerHandling>()->ActivateScreenSaver();
    break;

  case TMSG_RESETSCREENSAVER:
   m_components.GetComponent<CApplicationPowerHandling>()->m_bResetScreenSaver = true;
    break;
...
{
  const auto appPlayer = m_components.GetComponent<CApplicationPlayer>();
  bool renderGUI = m_components.GetComponent<CApplicationPowerHandling>()->GetRenderGUI();
  if (processEvents)
  {
...
bool CApplication::Cleanup()
{
  try
  {
    ...
    m_components.GetComponent<CApplicationSkinHandling>()->UnloadSkin();
    ...
    m_components.GetComponent<CApplicationPowerHandling>()->m_globalScreensaverInhibitor.Release();
    m_components.GetComponent<CApplicationPowerHandling>()->m_screensaverInhibitor.Release();
...
bool CApplication::OnMessage(CGUIMessage& message)
{
  ...
  case GUI_MSG_NOTIFY_ALL:
    {
      ...
      else if (message.GetParam1() == GUI_MSG_UI_READY)
      {
        ...
        if (message.GetSenderId() == WINDOW_SETTINGS_PROFILES)
          m_components.GetComponent<CApplicationSkinHandling>()->ReloadSkin(false);
      }
  ...
  case GUI_MSG_QUEUE_NEXT_ITEM:
    {
      ...
      if (appPlayer->QueueNextFile(file))
      {
        ...
        appPlayerControl->m_nextPlaylistItem = iNext;
      }
      else
      {
  ...
  case GUI_MSG_PLAYBACK_ENDED:
    ...
    { // just play the next item in the stack
      appPlayerControl->PlayFile(stackHelper->SetNextStackPartCurrentFileItem(), "", true);
      return true;
    }
    ResetCurrentItem();
...
bool CApplication::ExecuteXBMCAction(std::string actionStr, const CGUIListItemPtr &item /* = NULL */)
{
  ...
  else
  {
    ...
    { // an audio or video file
      const auto appPlayerControl = m_components.GetComponent<CApplicationPlayerControl>();
      appPlayerControl->PlayFile(item, "");
    }
    else
    {
...
void CApplication::ProcessSlow()
{
  ...
  m_components.GetComponent<CApplicationSkinHandling>()->ProcessSkin();
  ...
  m_components.GetComponent<CApplicationPowerHandling>()->CheckScreenSaverAndDPMS();
...
#endif
  {
    m_components.GetComponent<CApplicationPowerHandling>()->CheckShutdown();
  }
  ...
  const auto appPower = m_components.GetComponent<CApplicationPowerHandling>();
...
void CApplication::Restart(bool bSamePosition)
{
  ...
  if (!bSamePosition)
  {
    ...
    appPlayerControl->PlayFile(*m_itemCurrentFile, "", true);
    return ;
  }
  ...
  // reopen the file
  if (appPlayerControl->PlayFile(*m_itemCurrentFile, "", true))
...
void CApplication::SetLoggingIn(bool switchingProfiles)
{
  ...
  m_components.GetComponent<CApplicationSkinHandling>()->m_saveSkinOnUnloading = !switchingProfiles;
}
