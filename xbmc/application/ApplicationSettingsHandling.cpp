...
CApplicationSettingsHandling::
CApplicationSettingsHandling(CApplicationVolumeHandling& volumeHandling)
...
void CApplicationSettingsHandling::OnSettingChanged(const std::shared_ptr<const CSetting>& setting)
{
  ...
  if (appSkin && appSkin->OnSettingChanged(*setting))
    return;

  const auto appPower = components.GetComponent<CApplicationPowerHandling>();
  if (appPower && appPower->OnSettingChanged(*setting))
    return;
...
void CApplicationSettingsHandling::OnSettingAction(const std::shared_ptr<const CSetting>& setting)
{
  ...
  auto& components = CServiceBroker::GetAppComponents();
  const auto appPower = components.GetComponent<CApplicationPowerHandling>();
  if (appPower && appPower->OnSettingAction(*setting))
    return;
