...
bool CGUIDialogSubtitles::OnMessage(CGUIMessage& message)
{
  ...
  else if (message.GetMessage() == GUI_MSG_WINDOW_DEINIT)
  {
    ...
    if (appPlayer && appPlayer->IsPaused() && m_pausedOnRun)
      appPlayer->Pause();
...
void CGUIDialogSubtitles::OnInitWindow()
{
  ...
  if (!appPlayer)
    return;
  if (CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool(CSettings::SETTING_SUBTITLES_PAUSEONSEARCH) && !appPlayer->IsPaused())
  {
    appPlayer->Pause();
    m_pausedOnRun = true;
  }
...
bool CGUIDialogSubtitles::SetService(const std::string &service)
{
  if (service != m_currentService)
  {
    ...
    auto& components = CServiceBroker::GetAppComponents();
    const auto appPlayer = components.GetComponent<CApplicationPlayer>();
    if (!appPlayer || appPlayer->GetSubtitleCount() == 0)
