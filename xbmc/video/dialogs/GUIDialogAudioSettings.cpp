...
void CGUIDialogAudioSettings::FrameMove()
{
  ...
  float newVolume = appVolume ? appVolume->GetVolumeRatio() : 0.f;
...
void CGUIDialogAudioSettings::InitializeSettings()
{
  ...
  const auto appVolume = components.GetComponent<CApplicationVolumeHandling>();
  m_volume = appVolume ? appVolume->GetVolumeRatio() : 0.f;
                                              ...
                                              const std::string& settingId)
{
  ...
  auto& components = CServiceBroker::GetAppComponents();
  const auto appPlayer = components.GetComponent<CApplicationPlayer>();
  m_audioStream = appPlayer ? appPlayer->GetAudioStream() : 0;
  if (m_audioStream < 0)
    m_audioStream = 0;
                                                   ...
                                                   void* data)
{
  auto& components = CServiceBroker::GetAppComponents();
  const auto appPlayer = components.GetComponent<CApplicationPlayer>();
  return appPlayer ? appPlayer->IsPassthrough() : false;
}
                                                       ...
                                                       void* data)
{
  auto& components = CServiceBroker::GetAppComponents();
  const auto appPlayer = components.GetComponent<CApplicationPlayer>();
  int audioStreamCount = appPlayer ? appPlayer->GetAudioStreamCount() : 0;
