...
{
  StubDefinition(const std::string& path, bool res = true,
                 const std::string& tagPath = "", bool isFolder = false)
...
class DiscStubTest : public testing::WithParamInterface<StubDefinition>
                   , public testing::Test
{};

TEST_P(DiscStubTest, IsDiscStub)
{
  EXPECT_EQ(IsDiscStub(GetParam().item), GetParam().result);
}
...
{
  VideoClassifyTest(const std::string& path, bool res = true,
...
class VideoTest : public testing::WithParamInterface<VideoClassifyTest>
                , public testing::Test
{};

TEST_P(VideoTest, IsVideo)
{
  EXPECT_EQ(IsVideo(GetParam().item), GetParam().result);
}
...
TEST(TesVideoFileItemClassify, VideoExtensions)
{
