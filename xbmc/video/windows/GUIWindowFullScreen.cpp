...
bool CGUIWindowFullScreen::OnAction(const CAction &action)
{
  ...
  case ACTION_MOUSE_MOVE:
    ...
    {
      if (appPlayer && !appPlayer->IsInMenu())
      {
        TriggerOSD();
        return true;
      }
  ...
  case ACTION_ASPECT_RATIO:
    { // toggle the aspect ratio mode (only if the info is onscreen)
      if (m_dwShowViewModeTimeout.time_since_epoch().count() != 0)
      {
        ...
        appPlayer->SetRenderViewMode(vs.m_ViewMode, vs.m_CustomZoomAmount,
...
void CGUIWindowFullScreen::ClearBackground()
{
  auto& components = CServiceBroker::GetAppComponents();
  const auto appPlayer = components.GetComponent<CApplicationPlayer>();
  if (appPlayer && appPlayer->IsRenderingVideoLayer())
  ...
  case GUI_MSG_WINDOW_INIT:
    {
      ...
      auto& components = CServiceBroker::GetAppComponents();
      const auto appPlayer = components.GetComponent<CApplicationPlayer>();
      if (message.GetParam1() == WINDOW_INVALID && appPlayer && !appPlayer->IsPlayingVideo())
...
void CGUIWindowFullScreen::Process(unsigned int currentTime, CDirtyRegionList &dirtyregion)
{
  auto& components = CServiceBroker::GetAppComponents();
  const auto appPlayer = components.GetComponent<CApplicationPlayer>();
  if (appPlayer && appPlayer->IsRenderingGuiLayer())
    MarkDirtyRegion();
