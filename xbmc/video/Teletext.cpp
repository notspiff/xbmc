...
bool CTeletextDecoder::InitDecoder()
{
  ...
  m_txtCache = appPlayer? appPlayer->GetTeletextCache() : nullptr;
    ...
    int Ascender) // Ascender of font
{
  ...
  else if (Attribute->charset >= C_OFFSET_DRCS)
  {
    ...
    if (pcache)
    {
      ...
      if (!appPlayer)
        return 0;
      appPlayer->LoadPage((Attribute->charset & 0x10) ? m_txtCache->drcs : m_txtCache->gdrcs, Attribute->charset & 0x0f, drcs_data);
      unsigned char *p;
      if (Char < 23*2)
        p = drcs_data + 20*Char;
