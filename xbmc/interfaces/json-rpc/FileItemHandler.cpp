                                ...
                                CThumbLoader* thumbLoader /* = NULL */)
{
  ...
  if (item)
  {
    ...
    if (item->HasVideoInfoTag() && item->GetVideoContentType() == VIDEODB_CONTENT_TYPE::TVSHOWS)
    {
...
bool CFileItemHandler::FillFileItemList(const CVariant &parameterObject, CFileItemList &list)
{
  ...
  if (!file.empty() && (URIUtils::IsURL(file) || (CFileUtils::Exists(file) && !CDirectory::Exists(file))))
  {
    bool added = false;
