...
float CPlayerGUIInfo::GetSeekPercent() const
{
  ...
  float fPercent = fPercentPlayTime + fPercentPerSecond * (appPlayer ? appPlayer->GetSeekHandler().GetSeekSize() : 0);
...
std::string CPlayerGUIInfo::GetSeekTime(TIME_FORMAT format) const
{
  auto& components = CServiceBroker::GetAppComponents();
  const auto appPlayer = components.GetComponent<CApplicationPlayer>();
  if (!appPlayer || !appPlayer->GetSeekHandler().HasTimeCode())
    return std::string();
...
bool CPlayerGUIInfo::InitCurrentItem(CFileItem *item)
{
  auto& components = CServiceBroker::GetAppComponents();
  const auto appPlayer = components.GetComponent<CApplicationPlayer>();
  if (item && appPlayer && appPlayer->IsPlaying())
  {
...
bool CPlayerGUIInfo::GetBool(bool& value, const CGUIListItem *gitem, int contextWindow, const CGUIInfo &info) const
{
  ...
  switch (info.m_info)
  {
    ...
    {
      value = (appPlayer->GetPlayTempo() != 1.0f &&
               appPlayer->GetPlaySpeed() == 1.0f);
      return true;
    }
    case PLAYER_CACHING:
