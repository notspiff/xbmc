...
{
  if (!pItem || (pItem->m_bIsFolder && !IsAudio(*pItem)) ||
      IsPlayList(*pItem) || pItem->IsNFO() || IsInternetStream(*pItem))
    return false;
...
bool CMusicInfoLoader::LoadItemLookup(CFileItem* pItem)
{
  ...
  if ((!pItem->HasMusicInfoTag() || !pItem->GetMusicInfoTag()->Loaded()) && IsAudio(*pItem))
  {
