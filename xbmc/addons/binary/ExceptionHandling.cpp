...
void CAddonExceptionHandler::Handle(XbmcThreads::SegFaultException& e, bool onCrashProtectionStart/* = false*/)
{
  CLog::Log(LOGERROR,"EXCEPTION: %s", e.GetMessage());

  DestroyAddon(dynamic_cast<CAddon*>(e.GetCrashProtection()));
...
void CAddonExceptionHandler::Handle(const ADDON::UnimplementedException e, bool onCrashProtectionStart/* = false*/)
{
    CLog::Log(LOGERROR,"EXCEPTION: %s",e.GetMessage());
    fprintf(stderr, "EXCEPTION: %s\n",e.GetMessage());
}

void CAddonExceptionHandler::Handle(const XbmcCommons::Exception& e, bool onCrashProtectionStart/* = false*/)
{
    CLog::Log(LOGERROR,"EXCEPTION: %s",e.GetMessage());
    fprintf(stderr, "EXCEPTION: %s\n",e.GetMessage());
}

void CAddonExceptionHandler::HandleUnknown(std::string functionName, bool onCrashProtectionStart/* = false*/)
{
  CLog::Log(LOGERROR, "EXCEPTION: Unknown exception thrown from the call \"%s\"", functionName.c_str());
  fprintf(stderr, "EXCEPTION: Unknown exception thrown from the call \"%s\"\n", functionName.c_str());
}

void CAddonExceptionHandler::DestroyAddon(CAddon* addon)
{
  if (addon)
  {
