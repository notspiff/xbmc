...
void CPlayerCB_InfoTagVideo::Init(CB_PlayerLib *callbacks)
{
  callbacks->AddonInfoTagVideo.GetFromPlayer  = CPlayerCB_InfoTagVideo::GetFromPlayer;
  callbacks->AddonInfoTagVideo.Release        = CPlayerCB_InfoTagVideo::Release;
}

bool CPlayerCB_InfoTagVideo::GetFromPlayer(
        void*               addonData,
        PLAYERHANDLE        player,
        AddonInfoTagVideo*  tag)
{
  try
  {
    ...
    if (!helper || !player | !tag)
      throw ADDON::WrongValueException("CPlayerCB_InfoTagVideo - %s - invalid data (addonData='%p', player='%p', tag='%p')",
                                        __FUNCTION__, helper, player, tag);

    memset(tag, 0, sizeof(AddonInfoTagVideo));

    if (!g_application.m_pPlayer->IsPlayingVideo())
      throw ADDON::WrongValueException("CPlayerCB_InfoTagVideo - %s: %s/%s - Kodi is not playing any video file",
                                         __FUNCTION__,
    ...
    if (!infoTag)
    {
      CLog::Log(LOGERROR, "CPlayerCB_InfoTagVideo - %s: %s/%s - Failed to get video info",
                                         __FUNCTION__,
                                         TranslateType(helper->GetAddon()->Type()).c_str(),
                                         helper->GetAddon()->Name().c_str());
      return false;
...
void CPlayerCB_InfoTagVideo::Release(
        void*               addonData,
        AddonInfoTagVideo*  tag)
{
  try
  {
    CAddonCallbacks* helper = static_cast<CAddonCallbacks *>(addonData);
    if (!helper || !tag)
      throw ADDON::WrongValueException("CPlayerCB_InfoTagVideo - %s - invalid data (addonData='%p', tag='%p')",
