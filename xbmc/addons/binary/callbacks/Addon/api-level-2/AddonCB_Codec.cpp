...
kodi_codec CAddonCB_Codec::get_codec_by_name(void* addonData, const char* strCodecName)
{
  try
  {
    if (!addonData || !strCodecName)
      throw ADDON::WrongValueException("CAddonCB_Codec - %s - invalid data (addonData='%p', strCodecName='%p')",
...
void CAddonCB_Codec::free_demux_packet(void *addonData, DemuxPacket* pPacket)
{
  try
  {
    if (!addonData || !pPacket)
      throw ADDON::WrongValueException("CAddonCB_Codec - %s - invalid data (addonData='%p', pPacket='%p')",
