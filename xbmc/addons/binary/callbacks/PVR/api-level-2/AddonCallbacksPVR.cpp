...
CAddonCallbacksPVR::CAddonCallbacksPVR(CAddon* addon)
  : ADDON::IAddonCallback(addon, PVRLIB_apiLevel, PVRLIB_version),
...
A_DLLEXPORT CB_PVRLib* PVRLIB_V2_register_level(void *hdl, int level)
{
  ...
  {
    if (!hdl)
      throw ADDON::WrongValueException("CAddonCallbacksPVR - %s - invalid handler data", __FUNCTION__);

    cb = static_cast<CB_PVRLib*>(CAddonCallbacks::PVRLib_RegisterLevel(((AddonCB*)hdl)->addonData, level));
    if (!cb)
      throw ADDON::WrongValueException("CAddonCallbacksPVR - %s - can't get callback table from Kodi !!!", __FUNCTION__);
  }
...
A_DLLEXPORT void PVRLIB_V2_unregister(void *hdl, void* cb)
{
  try
  {
    if (!hdl || !cb)
